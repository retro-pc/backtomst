                      ...
                      while i<dwRead do
                        begin
                          ...
                          case ir[i].EventType of
                            ...
                            _MOUSE_EVENT:
                              begin
                                 if ir[i].Event.MouseEvent.dwEventFlags = 4{MOUSE_WHEELED} then
                                 begin

      				   if (ir[i].Event.MouseEvent.dwButtonState and $80000000) <> 0 then
                                      KeyCode := $0050{kbDown}{$0051}{kbPgDn}
                                   else
                                      KeyCode := $0048;{kbUp}{$0049}{kbPgUp}
                                   fillchar (ir[i], sizeof (ir[i]), 0);
                                   ir[i].EventType:=KEY_EVENT;
                                   With ir[i].Event.KeyEvent do
                                   begin
                                      bKeyDown := true;
                                      wVirtualScanCode:=KeyCode;
                                      wRepeatCount:=1;
                                   end;
                                   if assigned(KeyboardEventHandler) then
                                     KeyboardEventHandler(ir[i]);
